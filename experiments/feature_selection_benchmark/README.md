...
> **Каркас / WIP.** Сейчас в репозитории — только фундамент пакета:
> дерево директорий, упаковка (`pyproject.toml`), стабильные контракты
> данных (`Dataset`, `BenchmarkResult`, `FSMethod`, `MethodInfo`) и
> заглушки модулей с финальными сигнатурами.
>
> **Реальной логики ещё нет:** функции-заглушки (включая `run_benchmark`)
> поднимают `NotImplementedError`. Логика FS-методов, оркестрации, подбора
> гиперпараметров и хранилища реализуется в последующих эпиках
> (E-004…E-009).

Что уже сделано на уровне каркаса:

- дерево пакета по `ARCHITECTURE.md` §2;
- упаковка через `pyproject.toml` (PEP 517/518);
- стабильные контракты и публичные сигнатуры.
...
> Приведённый пример отражает целевой API. Сейчас это **каркас**:
> `run_benchmark` — заглушка, поднимающая `NotImplementedError`. Пример
> показывает, как вызов будет выглядеть после реализации в эпиках
> E-004…E-009.
