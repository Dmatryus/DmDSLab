...
class OutlierDetector:
    ...
    def detect_outliers(self,
        ...
        clean_indices = [i for i in range(len(clean_data)) 
    ...
    def _detect_iqr(self, 
                   data: np.ndarray,
                   multiplier: float = 1.5) -> OutlierResult:
        """
    ...
    def _detect_percentile(self,
                          data: np.ndarray,
                          lower_percentile: float = 5.0,
                          upper_percentile: float = 95.0) -> OutlierResult:
        """
    ...
    def remove_outliers(self,
                       data: Union[List[float], np.ndarray, pd.Series],
                       method: OutlierMethod = OutlierMethod.IQR,
                       **kwargs) -> Tuple[np.ndarray, OutlierResult]:
        """
    ...
    def analyze_multiple_runs(self,
                            ...
                            method: OutlierMethod = OutlierMethod.IQR,
                            **kwargs) -> Dict[str, Tuple[np.ndarray, OutlierResult]]:
        """
...
def compare_outlier_methods(data: Union[List[float], np.ndarray],
                          methods: Optional[List[OutlierMethod]] = None) -> pd.DataFrame:
    """
