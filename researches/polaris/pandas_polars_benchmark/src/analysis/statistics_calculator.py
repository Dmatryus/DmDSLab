...
class StatisticsCalculator:
    ...
    def calculate_descriptive_stats(
        self, data: Union[List[float], np.ndarray, pd.Series]
    ) -> DescriptiveStats:
        """
    ...
    def _calculate_confidence_interval(
        self, data: np.ndarray, mean: float, std: float
    ) -> Tuple[float, float]:
        """
    ...
    def test_normality(
        ...
        anderson_critical = dict(
            zip(["15%", "10%", "5%", "2.5%", "1%"], anderson_result.critical_values)
        )
        anderson_normal = {
        ...
        self.logger.debug(
            f"Тесты на нормальность: Shapiro p={shapiro_p:.4f}, "
    ...
    def calculate_stats_for_groups(
        self, groups: Dict[str, Union[List[float], np.ndarray]]
        ...
        for name, data in groups.items():
            try:
                ...
                results[name] = stats
                self.logger.info(
                    f"Рассчитаны статистики для {name}: "
    ...
    def create_summary_table(
        self, stats_dict: Dict[str, DescriptiveStats]
    ) -> pd.DataFrame:
        """
    ...
    def calculate_relative_metrics(
        self, base_stats: DescriptiveStats, compare_stats: DescriptiveStats
    ) -> Dict[str, float]:
        """
        ...
        mean_diff_pct = (
            (base_stats.mean - compare_stats.mean) / base_stats.mean * 100
        ...
        # Ускорение (speedup)
        speedup = (
            base_stats.mean / compare_stats.mean
            if compare_stats.mean != 0
            else float("inf")
        )
        ...
        ci_overlap = not (
            base_stats.confidence_interval[1] < compare_stats.confidence_interval[0]
            or compare_stats.confidence_interval[1] < base_stats.confidence_interval[0]
        )
