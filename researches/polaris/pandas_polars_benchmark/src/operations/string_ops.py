...
class StringReplaceOperation(DataAwareOperation):
    ...
    def execute_polars(self,
        ...
        try:
            ...
            if regex:
                result_df = df.with_columns(
                    pl.col(column).str.replace_all(pattern, replacement)
                    .alias(f"{column}_replaced")
                )
            else:
                result_df = df.with_columns(
                    pl.col(column).str.replace_all(pattern, replacement, literal=True)
                    .alias(f"{column}_replaced")
                )
...
class StringExtractOperation(DataAwareOperation):
    ...
    def execute_polars(self,
        ...
        try:
            ...
            # Извлекаем данные
            result_df = df.with_columns(
                pl.col(column).str.extract(pattern, group_index=1)
            ...
            if not lazy:
                successful_extracts = result_df[f"{column}_extracted"].is_not_null().sum()
                metadata.update({
                    "successful_extracts": int(successful_extracts),
                    "extract_ratio": successful_extracts / len(df) if len(df) > 0 else 0
                })
        ...
        try:
            ...
            if len(concat_cols) >= 2:
                ...
                for col in concat_cols[1:]:
                    result_df["concatenated"] = result_df["concatenated"] + separator + col
        ...
        try:
            ...
            if columns is None:
                ...
                if len(string_cols) < 2 and numeric_cols:
                    ...
                    return OperationResult(
                        success=False,
                        error=ValueError("Недостаточно колонок")
                    )
            ...
            if len(concat_exprs) >= 2:
                ...
                result_df = df.with_columns(
                    pl.concat_str(concat_exprs, separator=separator)
                    .alias("concatenated")
                )
            else:
                return OperationResult(
                    success=False,
                    error=ValueError("Недостаточно валидных колонок")
                )
            ...
            if not lazy:
                lengths = result_df.select(
                    pl.col("concatenated").str.len_chars()
                )["concatenated"]
                metadata.update({
...
class StringLengthOperation(DataAwareOperation):
    ...
    def execute_pandas(self,
        ...
        try:
            ...
            category_counts = result_df[f"{column}_length_category"].value_counts().to_dict()
    ...
    def execute_polars(self,
        ...
        try:
            ...
            result_df = df.with_columns([
                ...
                pl.when(pl.col(column).str.len_chars() <= 5).then(pl.lit("short"))
                .when(pl.col(column).str.len_chars() <= 10).then(pl.lit("medium"))
                .when(pl.col(column).str.len_chars() <= 20).then(pl.lit("long"))
            ...
            if not lazy:
                ...
                metadata.update({
                    "length_stats": {k: float(v) if k != 'min' and k != 'max' else int(v) 
