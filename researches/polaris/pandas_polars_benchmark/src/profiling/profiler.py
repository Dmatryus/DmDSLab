...
class ProfileResult:
    ...
    @classmethod
    def from_error(
        cls, operation_name: str, library: str, error: Exception, dataset_name: str = ""
    ) -> "ProfileResult":
        ...
        try:
            ...
            if isinstance(op_result, (pd.DataFrame, pl.DataFrame)):
                result.result_info = {
                    ...
                    "dtypes": (
                        str(op_result.dtypes) if hasattr(op_result, "dtypes") else None
                    ),
                }
    ...
    def _profile_isolated(
        ...
        try:
            ...
            result = ProfileResult(
                ...
                # Память
                peak_memory_mb=(
                    memory_stats.get("peak_memory_mb", 0) if memory_stats else 0
                ),
                avg_memory_mb=(
                    memory_stats.get("average_memory_mb", 0) if memory_stats else 0
                ),
                min_memory_mb=(
                    memory_stats.get("min_memory_mb", 0) if memory_stats else 0
                ),
                max_memory_mb=(
                    memory_stats.get("max_memory_mb", 0) if memory_stats else 0
                ),
                memory_samples=(
                    memory_stats.get("sample_count", 0) if memory_stats else 0
                ),
                # Статус
                success=True,
    ...
    **kwargs,
    ...
    with Profiler() as profiler:
        ...
        return profiler.profile_operation(
            wrapper, operation_name=operation_name, library=library
        )
