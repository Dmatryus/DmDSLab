...
def sample_data_split():
    ...
    return DataSplit(train=train_data, tuning=test_data, test=test_data.copy())


def test_basic_imputation(sample_data_split):
    """Тест базового заполнения пропусков"""
    imputer = DataImputer(num_strategy="mean", cat_strategy="most_frequent")
...
def test_iterative_imputation(sample_data_split):
    ...
    imputer = DataImputer(
        num_strategy="iterative", cat_strategy="constant", cat_constant="MISSING"
    )
...
def test_numeric_only_imputation():
    ...
    data = DataSplit(
        train=ModelData(pd.DataFrame({"num": [1, np.nan, 3]}), pd.Series([0, 1, 0])),
        tuning=ModelData(pd.DataFrame({"num": [np.nan, 5]}), pd.Series([1, 0])),
        test=ModelData(pd.DataFrame({"num": [6, np.nan]}), pd.Series([0, 1])),
    )
...
def test_categorical_only_imputation():
    ...
    data = DataSplit(
        train=ModelData(
            pd.DataFrame({"cat": ["a", np.nan, "a", np.nan]}), pd.Series(range(4))
        ),
        tuning=ModelData(pd.DataFrame({"cat": [np.nan, "b"]}), pd.Series([1, 0])),
        test=ModelData(pd.DataFrame({"cat": ["c", np.nan]}), pd.Series([0, 1])),
    )

    imputer = DataImputer(cat_strategy="constant", cat_constant="UNKNOWN")
    imputed_data = imputer.fit_transform(data)

    assert imputed_data.train.features["cat"].tolist() == [
        "a",
        "UNKNOWN",
        "a",
        "UNKNOWN",
